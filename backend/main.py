...
def _migrate_db():
    with engine.connect() as conn:
        ...
        if "watchlist_items" in tables:
            ...
            if "shares" not in wl_cols:
                conn.execute(text("ALTER TABLE watchlist_items ADD COLUMN shares FLOAT"))  # nosemgrep
            if "avg_cost" not in wl_cols:
                conn.execute(text("ALTER TABLE watchlist_items ADD COLUMN avg_cost FLOAT"))  # nosemgrep
            conn.commit()
        ...
        if "stock_analyses" in tables:
            sa_cols6 = {c["name"] for c in inspector.get_columns("stock_analyses")}
            for col, typ in [
                ("time_horizon_fit",           "VARCHAR"),
                ("time_horizon_reasoning",     "TEXT"),
                ("time_horizon_last_computed", "DATE"),
            ]:
                if col not in sa_cols6:
                    conn.execute(text(f"ALTER TABLE stock_analyses ADD COLUMN {col} {typ}"))  # nosemgrep
            conn.commit()

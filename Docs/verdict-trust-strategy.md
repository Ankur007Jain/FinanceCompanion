...
All of the first five features are additions to the **Verdict Agent** (claude-sonnet) prompt and new fields on `StockAnalysis`. No new tables, no new agents.

| Feature | Implementation |
|---------|---------------|
| Entry Quality signal | Verdict Agent computes from `rsi`, `range_position_pct`, `ma_200`, recent `day_change_pct` |
| 90-Day scenario | Claude generates Bull/Base/Bear + probabilities from analyst targets + conviction |
| Safe-to-Ignore rating | Rule-based + Claude judgment from earnings proximity, beta, leverage flag |
| Position sizing | Formula-based output from conviction + risk level |
| Don't Panic protocol | New nightly check: if `current_price` dropped >15% since last BUY verdict → trigger intervention note |
| Historical pattern match | Query `stock_analyses` history once sufficient data exists |

New `StockAnalysis` fields needed:
- `entry_quality` — `"GREAT" | "FAIR" | "WAIT"`
- `scenario_bull`, `scenario_base`, `scenario_bear` — string descriptions
- `scenario_bull_pct`, `scenario_base_pct`, `scenario_bear_pct` — float % return estimates
- `scenario_bull_prob`, `scenario_base_prob`, `scenario_bear_prob` — integer probabilities (sum to 100)
- `hold_and_forget_rating` — `"HOLD_AND_FORGET" | "CHECK_MONTHLY" | "WATCH_CLOSELY"`
- `position_size_suggestion` — string e.g. "5–8% of portfolio"
- `dont_panic_note` — string, populated only when price dropped >15% since last BUY
...
> **NFLX — BUY**
> Entry Quality: GREAT · Hold-and-Forget: ✓ · Position: 6% of portfolio
> Bull +24% / Base +9% / Bear -11%
